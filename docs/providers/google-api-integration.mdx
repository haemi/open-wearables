...
<Note>
  `google_health` is this provider's name everywhere — in payloads, in `?provider=` filters, and in every URL. Before the split it was reachable at `/google/`; the webhook endpoint still serves that path so subscribers already registered with Google keep delivering, but nothing else accepts it.
</Note>
...
<Steps>
  <Step title="Enable the Health API and create an OAuth client">
    ...
    <Warning>
    **Upgrading from before the provider split?** Your client is registered under the old `/oauth/google/callback` path. Add the `google_health` URI above — Google accepts several authorized redirect URIs, so adding it ahead of the deploy causes no downtime, and existing connections keep syncing throughout (token refresh does not use a redirect URI). Only new connections need it.
    </Warning>
